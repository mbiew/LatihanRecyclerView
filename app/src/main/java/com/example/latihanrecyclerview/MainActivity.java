...
public class MainActivity extends AppCompatActivity {
    ...
    private List<Song> songList; private SongAdapter songAdapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        for (int i = 0; i < names.length; i++) {
            ...
            Song song = new Song(names[i], singers[i], i + 1, pics[i]); songList.add(song);
        }

        songAdapter = new SongAdapter(songList);
        mRecyclerView.setAdapter(songAdapter); songAdapter.notifyDataSetChanged();
        mRecyclerView.addOnItemTouchListener(new RecyclerItemClickListener(this,
                new RecyclerItemClickListener.OnItemClickListener() { @Override
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            case R.id.item_grid:
//	use this in case of gridlayoutmanager mLayoutManager = new GridLayoutManager(this, 2); mRecyclerView.setLayoutManager(mLayoutManager); break;
